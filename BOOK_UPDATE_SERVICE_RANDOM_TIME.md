...
Сервис `BookUpdateService` был изменён для запуска только в случайное время в промежутке от **0:00 до 6:00 по московскому времени (GMT+3)**.
...
- **Теперь**: Запуск в случайное время между 0:00 и 6:00 по московскому времени
...
- Если текущее время меньше 6:00 MSK - планирует запуск на сегодня
- Если текущее время больше 6:00 MSK - планирует запуск на следующий день
- Генерирует случайное время в диапазоне от 0 до 360 минут (0:00 - 6:00)
...
- После принудительного запуска следующий плановый запуск также планируется на случайное время между 0:00 и 6:00 MSK
...
3. **Диапазон**: Строго с 0:00:00 до 5:59:59 по московскому времени
...
- Следующий запуск будет запланирован на **24.12.2025 между 0:00 и 6:00 MSK**
- Например, случайно может быть выбрано: **24.12.2025 03:27 MSK**

Если сейчас **24.12.2025 02:30 MSK** (уже в окне 0:00-6:00):
- Следующий запуск будет запланирован на **25.12.2025 между 0:00 и 6:00 MSK**
...
3. Проверить поле `nextRunTimeUtc` - оно должно указывать на время между 21:00 и 3:00 UTC (что соответствует 0:00-6:00 MSK)
